...
func TestStdlibAdapter_Log(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			w := &strings.Builder{}
			logger := golfstdlib.New(stdliblog.New(w, "", 0), tt.formatter)
			logger.Log(tt.kvs...)
...
func TestStdlibAdapter_Log_FormatterError(t *testing.T) {
	...
	logger := golfstdlib.New(stdliblog.New(ioutil.Discard, "", 0), errorFormatter)
...
func TestStdlibAdapter_Log_LoggerError(t *testing.T) {
	w := errorWriter{Err: errors.New("some error")}
	logger := golfstdlib.New(stdliblog.New(w, "", 0), nil)
	actual := logger.Log("key", "value")
